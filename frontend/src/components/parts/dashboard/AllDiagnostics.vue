...
<template>
  <div class="mb-10">
    <div class="flex items-center justify-between mb-6">
      <div class="space-y-1">
        <span class="badge badge-primary badge-outline font-bold tracking-widest text-[10px] h-auto py-1 px-3">HISTORIAL</span>
    ...
    <div v-else class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative">
      ...
      <div v-if="isPaginating" class="absolute inset-0 z-10 bg-white/50 backdrop-blur-sm flex items-center justify-center">
              ...
              <td>
                <div v-if="analysis.status === 'COMPLETED' && analysis.detections" class="flex gap-3">
                  <template v-for="(count, severity) in countSeverities(analysis.detections)" :key="severity">
                    <div v-if="count > 0" class="flex items-center gap-1.5" :title="severity === 'critical' ? 'Críticos' : severity === 'warning' ? 'Seguimiento' : 'Óptimos'">
