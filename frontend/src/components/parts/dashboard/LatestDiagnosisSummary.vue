  ...
  <div class="flex-1 md:col-span-5 flex justify-around">
    <div class="card bg-white border border-slate-200 shadow-sm w-full">
      <div class="card-body p-6">
        ...
        <div v-if="latestAnalysis" class="space-y-5">
          <div class="flex items-start gap-4">
            <div class="w-full sm:w-32">
              <div class="relative group w-full aspect-square rounded-xl overflow-hidden border border-slate-100">
                <img v-if="latestImageUrl" :src="latestImageUrl" :alt="`Radiografía - ${latestAnalysis.fileName}`"
                     class="w-full h-full object-cover"/>
                <div v-else class="w-full h-full bg-slate-100 flex items-center justify-center text-slate-400">No hay
                  imagen
                </div>
              </div>
            </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-center">
            ...
            <div class="rounded-xl border border-slate-100 bg-white p-3">
              <p class="text-[10px] font-black text-slate-400 uppercase tracking-tighter">Confianza media</p>
              <p class="mt-1 text-2xl font-black text-slate-900">{{ averageConfidence ?? '--' }}%</p>
            </div>
          </div>
        ...
        <div v-else class="rounded-xl border border-dashed border-slate-200 bg-slate-50 p-5 text-slate-600">
          <p class="font-semibold">Aún no hay un diagnóstico reciente para resumir.</p>
          <p class="text-sm mt-1">Sube una radiografía o fotografía para ver aquí el último resultado analizado.</p>
        </div>
      </div>
    </div>
  </div>
